...
get_all_tn_tracts = function()
{
  tn.counties = acs.fetch(geography    = geo.make(state = 47, county = "*"),
                          table.number = "B01003")

  geo.make(state  = "TN",
           county = as.numeric(geography(tn.counties)[[3]]),
           tract  = "*")
}
...
get_tn_tract_acs_data = function(tableId, endyear=2012, span=5, column_idx=-1)
{
  acs.data = acs.fetch(geography    = get_all_tn_tracts(),
...
#' Create a choropleth map of Tennessee Census Tracts from ACS data
#'
#' Creates a choropleth of Tennessee Census Tracts the US Census' American Community Survey (ACS) data.
...
#' tn_tract_choropleth_acs("B19301", county_zoom=6075)
#'
#' # aggregate public assistance income, san francisco
#' tn_tract_choropleth_acs("B19067", county_zoom=6075)
#' }
tn_tract_choropleth_acs = function(tableId, endyear=2011, span=5, num_colors=7, tract_zoom=NULL, county_zoom=NULL)
{
  acs.data = get_tn_tract_acs_data(tableId, endyear, span)
  tn_tract_choropleth(acs.data[['df']], acs.data[['title']], "", num_colors, tract_zoom, county_zoom)
}
